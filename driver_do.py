import os
from utils import data_prepare_do_bm as dp
from memory_profiler import profile
from core_utils.config_manager import DBManager
from src import tuning_do_bm
from src import training_do_bm
from src import inference_do_bm
import pandas as pd
import tracemalloc
...
end_date = str(pd.Timestamp(forecast_for_last_week) - pd.Timedelta(days=1 * 7))
default_config = {
        'op_name'           : 'inference',
        ...
        "prepared_data_table_name": 'temp_prepared_data_do',
...
def prepare_config(config, tenant_id, env, infer_start_date, horizon, tenant_type="B&M"):
    ...
    logger.info(
        f'------ DO Job Started for tenant={config["tenant_id"]} at {pd.Timestamp("today").strftime("%Y-%m-%d %H:%M:%S.%02f")[:-4]}-----')
...
def tune_train_infer(config, category):
    ...
    train_data, infer_data = dp.fetch_prepared_data_from_source(config, category, formatted_category, conn,
                                                                config["data_save_method"])
    ...
    tuning_do_bm.tune_for(config,tune_run_id, train_data)
    ...
    training_do_bm.train_for(config,tune_run_id, train_data)
    ...
    inference_do_bm.infer_for(config, infer_data, tune_run_id)
    ...
    with open("output_do.txt", "a") as f:
        print(f"--full process completed for category: {category}  in {(time.time() - start_time)} seconds", file=f)
...
def driver_new(config):
    ...
    # cat_pairs_full_list =['NATURAL  ORGANIC DAIRY']
                                                            ...
                                                            # '01 Propane Prefills','15 Salty Snacks']
                                                            #'23 Other Publications', '14 Packaged Bread']
    ...
    try:
        os.remove(f"no_lumpy_itmt_cat_{config['tenant_id']}.txt")
    ...
    exclude_cat_list = []
    try:
        with open(f"no_lumpy_itmt_cat_{config['tenant_id']}.txt", "r") as f:
            for line in f:
                x = line[:-1]
    ...
    with open("output_do.txt", "a") as f:
        print(f"DO for tenant_id : {config['tenant_id']} completed in {(time.time() - start_time) / 60} minutes", file=f)
